...
- Camera — yaw keeps the sprite convention (angle between the bar axis and the screen; 18° assembled, 38° exploded), pitch is 14° above the bar, a 22° lens keeps the near-orthographic look, and the fit distance accounts for the bar's reach toward the eye when yawed so the near end always clears the frame.
