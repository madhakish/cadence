...
- Material — the ground is a flat surface (no decorative gradient); a raised surface carries a hairline, a one-pixel edge light (`--edge` / `Theme.edge`), and a shadow token that lightens on the light themes. Nothing else fakes depth.
- Load numeral — every stated load (the working set, the achieved total) is one tabular display treatment (`.load-numeral`); the surface only chooses its size. Radii are 4 and 2 on both clients; steppers and tags are squared, and only the platform switch stays a pill.
- Theme safety — no control paints a hard-coded dark or iOS-system colour; verdict fills take the ground colour as ink so they flip with the theme, and semantic tints are mixed from their own token.
