...
| Plate metadata, exact solver stack and denominations | Shared `BarbellView`/`barbellSVG`, reverse ordering, exact 1.25, complete-bar and total contracts; #204 removes duplicate announcements | Photographic faces and shared 38° inspection are now implemented; see [current renderer evidence](BARBELL-INSPECTION.md). Native device capture remains an explicit verification gate. |
