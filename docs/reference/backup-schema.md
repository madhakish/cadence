...
## Version 6 climbed flights

Version 6 adds one optional per-set field, `flights`, for conditioning a
...
- The key is **emitted only when set**, like `inclinePercent` and
  ...
  re-export byte-for-byte identically to their version-5 form.

The version still has to move even though the field is optional and additive:
a version-5 importer parses the bundle happily and silently drops the count,
...
No field shapes changed and no field was removed, so a version-5 bundle
restores under version 6 untouched, with `flights` absent everywhere.
