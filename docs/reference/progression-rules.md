...
- *Below plan* means fewer at-plan working sets than prescribed, with a
  tolerance of **half a rounding step** (small entry noise is fine; a full
  plate step down is a drop). Compare actual performed mass with the
  achievable `plannedWeightLb`, not the theoretical `targetWeightLb`.
  No plate denomination substitutes for another: 45 lb + 100 kg is
  **265.462 lb**, so it meets a generated 265.462 lb plan but not a physical
  275 lb plan ([INV-PLATES-USE-MASS](invariants.md)). The same numeric rule
  applies to barbells, machines, and dumbbells. Extra back-off sets beyond
  the prescription never hurt the grade.
