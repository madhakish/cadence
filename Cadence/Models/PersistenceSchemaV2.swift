...
/// Frozen model shape shipped by PR #72. That build still labeled this shape
/// V1; retaining its checksum here lets a store created by that build migrate
/// to the corrected final schema as well.
enum CadenceSchemaV2: VersionedSchema {
    ...
    static var models: [any PersistentModel.Type] {
        [
            ...
            ProgramLift.self, ProgramAccessory.self,
        ]
    }
    ...
    final class BodyweightEntry {
        ...
        init(date: Date = .now, weightLb: Double) { self.date = date; self.weightLb = weightLb }
    }
    ...
    final class ProteinEntry {
        ...
        init(date: Date = .now, grams: Double, label: String) {
            self.date = date; self.grams = grams; self.label = label
        }
    }
    ...
    final class CheckIn {
        ...
        init(date: Date = .now, siteRaw: String, response: String) {
            self.date = date; self.siteRaw = siteRaw; self.response = response; self.note = ""
        }
    }
    ...
    final class Milestone {
        ...
        init(date: Date = .now, exerciseName: String? = nil, kindRaw: String, label: String) {
            self.date = date; self.exerciseName = exerciseName; self.kindRaw = kindRaw; self.label = label
        }
    }
    ...
    final class Program {
        ...
        var createdAt: Date
        init(name: String) {
            self.name = name; self.focusRaw = "strength"; self.cycleNumber = 1
            self.currentWeek = 1; self.nextDayIndex = 0; self.roundingLb = 5
            self.isActive = true; self.days = []; self.createdAt = .now
        }
    }
    ...
    final class ProgramDay {
        ...
        var order: Int
        var program: Program?
        @Relationship(deleteRule: .cascade, inverse: \ProgramLift.day) var lifts: [ProgramLift]
        @Relationship(deleteRule: .cascade, inverse: \ProgramAccessory.day) var accessories: [ProgramAccessory]
        init(name: String, order: Int) { self.name = name; self.order = order; self.lifts = []; self.accessories = [] }
    }
    ...
    final class ProgramLift {
        ...
        init(exerciseName: String) {
            self.exerciseName = exerciseName; self.roleRaw = "main"; self.baseWeightLb = 100
            self.estimatedMaxLb = 120; self.stallCount = 0; self.lastIncrementLb = 0
        }
    }
    ...
    final class ProgramAccessory {
        ...
        var day: ProgramDay?
        init(exerciseName: String) {
            self.exerciseName = exerciseName; self.sets = 3; self.minReps = 8
            self.maxReps = 12; self.currentReps = 8; self.weightLb = 0
            self.incrementLb = 5; self.stallCount = 0
        }
    }
}
