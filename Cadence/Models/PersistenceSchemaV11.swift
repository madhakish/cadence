...
/// Frozen schema shipped as V11. V11 is the V10 model plus the optional
/// stable-identity columns (`Exercise.id`, `WorkoutSession.programTemplateID`,
/// `SessionExercise.exerciseID`, `LiftTrack.exerciseID`, `Milestone.exerciseID`,
/// `Program.templateID`, `ProgramLift.exerciseID`,
/// `ProgramAccessory.exerciseID`), and nothing else. These declarations
/// preserve that checksum and must never be edited.
enum CadenceSchemaV11: VersionedSchema {
    ...
    static var models: [any PersistentModel.Type] {
        [
            ...
            // Unchanged since its V10 introduction, so the live class IS the
            // frozen shape (same convention as the V10 snapshot).
