...
/// Frozen schema shipped as V6 — the last version before vertical pulling was
/// promoted to a main lift. V6 is where `SetEntry.flights` arrived, the count
/// for conditioning a machine measures in climbed floors.
///
/// These declarations exactly preserve V6's checksum and **must never be
/// edited**. Freezing it here is what makes the V7 migration provable, the same
/// way freezing V5 made V6's provable. V6 is V5 plus `SetEntry.flights` and
/// nothing else — every other model is byte-identical to the V5 snapshot.
