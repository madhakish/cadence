...
/// Frozen schema shipped before conditioning learned to count climbed flights.
/// These declarations intentionally duplicate the released V4 persisted shape
/// so SwiftData can recognize an installed V4 store and migrate it forward.
...
enum CadenceSchemaV4: VersionedSchema {
    ...
    final class CoachingDecision {
        ...
        init(id: String = UUID().uuidString, programID: String, ruleID: String) {
            self.id = id
            self.date = .now
