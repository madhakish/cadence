...
- Do not build object hierarchies when a struct and a couple of functions will
  do.

Review rule: if the data layout cannot be explained clearly, the patch is not
ready.
...
Review rule: unnecessary generality is a bug. Overengineered scaffolding is
bogus shit.
...
Review rule: every changed line must have a direct reason to exist. Otherwise
it is random churn.
...
- **Enterprise sludge** — layers of factories, builders, managers, and config
  knobs for a trivial task.
- **Special-case insanity** — a pile of conditionals that should have been
  fixed in the data model.
- **Voodoo programming** — barriers, loops, helpers, or retries added without
  understanding.
...
Merge project-specific instructions below these principles if needed. Do not
dilute the doctrine into bureaucratic sludge.
