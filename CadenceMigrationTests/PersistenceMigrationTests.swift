...
final class PersistenceMigrationTests: XCTestCase {
    ...
    private func assertActualShippedStore(environmentKey: String) throws {
        // CI sets CADENCE_REQUIRE_SHIPPED_STORES (through xcodebuild's
        // TEST_RUNNER_ prefix) so that a fixture which fails to reach this
        // process is a failure, never a skip counted as coverage (#213). A
        // local run without generated fixtures still skips honestly.
        let environment = ProcessInfo.processInfo.environment
        let required = environment["CADENCE_REQUIRE_SHIPPED_STORES"] == "1"
        guard let sourcePath = environment[environmentKey] else {
            if required {
                XCTFail("\(environmentKey) did not reach XCTest; the shipped-store lineage was not exercised")
                return
            }
        ...
        guard FileManager.default.fileExists(atPath: source.appendingPathComponent("default.store").path) else {
            XCTFail("\(environmentKey) points at \(sourcePath) but it holds no default.store")
            return
        }
        let directory = FileManager.default.temporaryDirectory
