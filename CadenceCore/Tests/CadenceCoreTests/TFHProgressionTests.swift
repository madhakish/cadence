...
final class TFHProgressionTests: XCTestCase {
    private var anchor: TFHAnchor {
        TFHAnchor(id: "anchor", exerciseId: "exercise", weightLb: 42, reps: [3,3,3,3],
                  minReps: 3, maxReps: 5, incrementLb: 5, mode: "repsFirst",
    ...
    func testRecoveryAndAmbiguousHistory() throws {
        ...
        XCTAssertEqual(TFHProgression.project(anchor: anchor, exposures: duplicates, cycle: 1, rotation: 2)?.reps, anchor.reps)
        XCTAssertEqual(TFHProgression.project(anchor: anchor, exposures: duplicates, cycle: 1, rotation: 4)?.state, "recover")
    }

    func testBodyweightAndMaintenanceDoNotBecomeLoadIncreases() {
        var bw = anchor
        bw.mode = "bodyweight"; bw.weightLb = 0; bw.incrementLb = 0; bw.loadBasis = .bodyweight
